####### DADA2 Processing for barcode identification and relative abundance calculations for ME4 ####### 

library(dada2)
experiment <- 'ME4'

####### Filter and Trim #######
#path of forward and reverse reads for experiment ME4
...
#get sample names
sample.names <-  gsub('_EKDL220018360-1A_HKHMFDSX5_L2_1.fq.gz','',list.files(filtpathF, pattern="fq.gz", full.names = F))
sample.namesR <- gsub('_EKDL220018360-1A_HKHMFDSX5_L2_2.fq.gz','',list.files(filtpathR, pattern="fq.gz", full.names = F))
...
### DADA2 Processing for barcode identification and relative abundance calculations for LB012 ###
...
#path of forward and reverse reads for experiment ME4
pathF <-"~/Desktop/PhD/Publications/Library_Paper/Data_analysis/Barcode_sequencing/Raw_data/ME4/Forward_reads"
pathR <-"~/Desktop/PhD/Publications/Library_Paper/Data_analysis/Barcode_sequencing/Raw_data/ME4/Reverse_reads"
...
#save ASVs (barcodes) and their abundances in each sample
seqtab_barcodes <- tibble::rownames_to_column(as.data.frame(t(seqtab)), "barcode")
write.csv(seqtab_barcodes, file=paste(experiment,'_barcode_abundances.csv',sep=''),row.names=FALSE)
